...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:banana_juice) { described_class.new('banana_juice') }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(banana_juice.type).to eq('banana_juice')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(banana_juice.ounces).to be(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:mate) { described_class.new('mate') }
      # remove the 'x' before running this test
      it 'is full' do
        expect(mate).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:coffee) { described_class.new('coffee', 12) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(coffee).not_to be_full
      end
    end
  end
end
