...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:number_end) { double('random_number', value: 4) }
      subject(:game_end) { described_class.new(0, 9, number_end, 4) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        expect(low_guess_game.min) == 5
      end

      it 'does not update max' do
        expect(low_guess_game.max) == 9
      end
    end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        expect(high_guess_game.min) == 0
      end

      it 'updates max to 8' do
        expect(high_guess_game.max) == 8
      end
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:range_game) { described_class.new(5, 8, number_range, 7) }

      before do
        range_game.update_range
      end

      it 'updates min to the same value as max' do
        minimum = range_game.update_range
        expect(minimum).to eq(8)
      end

      it 'does not update max' do
        maximum = range_game.update_range
